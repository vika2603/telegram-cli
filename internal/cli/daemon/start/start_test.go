package start_test

import (
	"context"
	"errors"
	"testing"

	"github.com/stretchr/testify/require"

	"github.com/vika2603/telegram-cli/internal/cli/daemon/start"
	"github.com/vika2603/telegram-cli/internal/command"
	"github.com/vika2603/telegram-cli/internal/daemon"
	"github.com/vika2603/telegram-cli/internal/ui"
)

type fakeMgr struct {
	installed bool
	startErr  error
	started   int
}

func (f *fakeMgr) Platform() string            { return "fake" }
func (f *fakeMgr) Install(daemon.Config) error { return nil }
func (f *fakeMgr) Uninstall() error            { return nil }
func (f *fakeMgr) Start() error                { f.started++; return f.startErr }
func (f *fakeMgr) Stop() error                 { return nil }
func (f *fakeMgr) Restart() error              { return nil }
func (f *fakeMgr) Status() (*daemon.Status, error) {
	return &daemon.Status{Installed: f.installed, Platform: "fake"}, nil
}

func TestRun_RequiresInstall(t *testing.T) {
	ios, _, _, _ := ui.Test()
	m := &fakeMgr{installed: false}
	err := start.Run(context.Background(), &start.Options{
		Account:   "alice",
		IOStreams: ios,
		NewMgr:    func(string) (daemon.Manager, error) { return m, nil },
	})
	require.ErrorIs(t, err, command.ErrPrecondition)
	require.Zero(t, m.started)
}

func TestRun_DelegatesToManager(t *testing.T) {
	ios, _, _, _ := ui.Test()
	m := &fakeMgr{installed: true}
	err := start.Run(context.Background(), &start.Options{
		Account:   "alice",
		IOStreams: ios,
		NewMgr:    func(string) (daemon.Manager, error) { return m, nil },
	})
	require.NoError(t, err)
	require.Equal(t, 1, m.started)
}

func TestRun_ReturnsManagerErrorVerbatim(t *testing.T) {
	ios, _, _, _ := ui.Test()
	want := errors.New("kickstart failed")
	m := &fakeMgr{installed: true, startErr: want}
	err := start.Run(context.Background(), &start.Options{
		Account:   "alice",
		IOStreams: ios,
		NewMgr:    func(string) (daemon.Manager, error) { return m, nil },
	})
	require.ErrorIs(t, err, want)
}

func TestRun_RejectsEmptyAccount(t *testing.T) {
	ios, _, _, _ := ui.Test()
	err := start.Run(context.Background(), &start.Options{
		IOStreams: ios,
		NewMgr:    func(string) (daemon.Manager, error) { return &fakeMgr{}, nil },
	})
	require.ErrorIs(t, err, command.ErrUsage)
}
